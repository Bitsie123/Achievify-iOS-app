...
class cellclassInterval: UITableViewCell {}
...
class IntervalViewController: UIViewController {
    ...
    // Data source for Interval distance options
    ...
    // Data source for Interval pace ranges
    ...
    override func viewDidLoad() {
        ...
        tableView.register(cellclassInterval.self, forCellReuseIdentifier: "Cell")
    }
    ...
    @IBAction func onClickSelect(_ sender: Any) {
        dataSource = ["5:30-6:00 min/km", "5:00-5:30 min/km", "4:30-5:00 min/km", "4:00-4:30 min/km", "3:30-4:00 min/km"]
    ...
    @IBAction func onSave(_ sender: Any) {
        if validateTextFields() {
            ...
            taperVC.intervalPlanHeartRate = "152-171 bpm" // Pass average Heart Rate value
            taperVC.intervalPlanPace = btnRange.title(for: .normal) // Pass the selected range
            taperVC.intervalPlanDistance = btnDistance.title(for: .normal) // Pass the selected distance
    ...
    @IBAction func onVariationTapped(_ sender: Any) {
        let alertController = UIAlertController(title: "Optional Variations", message: "Add Strides: 5 x 400-meter intervals at 4-4:10 min/km with 2 minutes of recovery at 6:30-7:00 min/km in between.\n\nBreaks: 2-minute recovery jog between intervals.", preferredStyle: .alert)
